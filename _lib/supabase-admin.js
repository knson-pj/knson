...
async function updateStaff(userId, patch = {}) {
  const authUser = await getAuthUser(userId).catch(() => null);

  if (authUser?.id) {
    await updateAuthUser(userId, patch);
  } else if (patch.email != null || patch.password != null || patch.assignedRegions != null) {
    const err = new Error('해당 계정은 로그인 계정이 연결되지 않은 기존 프로필입니다. 이메일/비밀번호/지역 배정 변경은 연결된 계정에서만 가능합니다.');
    err.status = 400;
    throw err;
  }
  ...
  const item = await getStaff(userId);
  if (item) return item;
  ...
  return {
    ...
    role: normalizeRole(patch.role) || 'staff',
    position: String(patch.position || '').trim(),
    phone: String(patch.phone || '').replace(/[^\d]/g, ''),
    assignedRegions: Array.isArray(patch.assignedRegions) ? patch.assignedRegions.filter(Boolean).map((v) => String(v)) : [],
