...
// KST(UTC+9) 기준으로 YYYY-MM-DD 추출 (Vercel 서버는 UTC 로 동작하므로 명시적 변환 필요)
function toKstDateKey(dateLike) {
  const d = dateLike instanceof Date ? dateLike : new Date(dateLike);
  if (Number.isNaN(d.getTime())) return '';
  // UTC 시각에 9시간 더해서 KST 환산 → 그 후 UTC 메서드로 추출 (이중 환산 방지)
...
function sameDay(dateLike, dateKey) {
  ...
  const key = toKstDateKey(dateLike);
