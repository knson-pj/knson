(() => {
  ...
  function setFieldLabel(shell, fallbackText) {
    if (!shell) return;
    const labelEl = shell.querySelector('label, .label, .field-label, .input-label, .modal-label, .aem-label');
    if (labelEl) labelEl.textContent = fallbackText;
  }
