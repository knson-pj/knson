(() => {
  ...
  function cacheEls() {
    Object.assign(els, {
      ...
      // 신규 업데이트 — 이번주 / 이번달 (2026-05-08)
      sumWeekAuction: $("#sumWeekAuction"),
      sumWeekOnbid: $("#sumWeekOnbid"),
      sumWeekRealtor: $("#sumWeekRealtor"),
      sumWeekDirect: $("#sumWeekDirect"),
      sumWeekGeneral: $("#sumWeekGeneral"),
      sumMonthAuction: $("#sumMonthAuction"),
      sumMonthOnbid: $("#sumMonthOnbid"),
      sumMonthRealtor: $("#sumMonthRealtor"),
      sumMonthDirect: $("#sumMonthDirect"),
      sumMonthGeneral: $("#sumMonthGeneral"),
  ...
  function normalizeOverviewPayload(res) {
    ...
    if (candidate.summary && typeof candidate.summary === 'object') {
      return {
        ...
        weekly: {
          total: Number(candidate?.weekly?.total || 0),
          auction: Number(candidate?.weekly?.auction || 0),
          onbid: Number(candidate?.weekly?.onbid || 0),
          realtor: Number(candidate?.weekly?.realtor || 0),
          realtor_naver: Number(candidate?.weekly?.realtor_naver || 0),
          realtor_direct: Number(candidate?.weekly?.realtor_direct || 0),
          general: Number(candidate?.weekly?.general || 0),
        },
        monthly: {
          total: Number(candidate?.monthly?.total || 0),
          auction: Number(candidate?.monthly?.auction || 0),
          onbid: Number(candidate?.monthly?.onbid || 0),
          realtor: Number(candidate?.monthly?.realtor || 0),
          realtor_naver: Number(candidate?.monthly?.realtor_naver || 0),
          realtor_direct: Number(candidate?.monthly?.realtor_direct || 0),
          general: Number(candidate?.monthly?.general || 0),
        },
