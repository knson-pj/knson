...
function isDuplicatePropertyConstraintError(error) {
...
function createDuplicatePropertyError() {
  const err = new Error('동일 물건이 이미 등록되어 있습니다');
  err.status = 409;
  err.code = 'DUPLICATE_PROPERTY';
  return err;
}
...
async function handleSupabasePublicListing(res, payload) {
  ...
  if (existing) {
    ...
    let rows;
    try {
      rows = await supabaseRest(`/rest/v1/properties?${targetCol}=eq.${encodeURIComponent(targetId)}`, {
        method: 'PATCH',
        headers: { Prefer: 'return=representation' },
        json: patch,
      });
    } catch (err) {
      if (isDuplicatePropertyConstraintError(err)) throw createDuplicatePropertyError();
      throw err;
    }
  ...
  let created;
  try {
    created = await supabaseRest('/rest/v1/properties', {
      method: 'POST',
      headers: { Prefer: 'return=representation' },
