...
async function querySingleProperty(filterQuery) {
  const rows = await supabaseFetch(`/rest/v1/properties?select=*&${filterQuery}&limit=1`).catch(() => null);
...
async function getPropertyRow(propertyRef) {
  ...
  const direct = await querySingleProperty(`id=eq.${encodeURIComponent(rawRef)}`);
  if (direct) return direct;

  const numericId = parsePropertyId(rawRef);
  if (numericId && String(numericId) !== rawRef) {
    const numeric = await querySingleProperty(`id=eq.${encodeURIComponent(numericId)}`);
    if (numeric) return numeric;
  }

  const ref = encodeOrValue(rawRef);
  const byGlobal = await querySingleProperty(`global_id=eq.${ref}`);
  if (byGlobal) return byGlobal;

  const byItemNo = await querySingleProperty(`item_no=eq.${ref}`);
...
function getResolvedPropertyGlobalId(row) {
  const raw = row && typeof row.raw === 'object' ? row.raw : {};
  return trimRef(row?.global_id || raw.global_id || raw.globalId || '');
}

async function syncLegacyPropertyPhotos(propertyId, propertyGlobalId) {
  const pid = trimRef(propertyId);
  const gid = trimRef(propertyGlobalId);
  if (!pid || !gid) return;
  await patchPhotoRows(`property_global_id=eq.${encodeURIComponent(gid)}&property_id=neq.${encodeURIComponent(pid)}&deleted_at=is.null`, { property_id: pid, updated_at: new Date().toISOString() }).catch(() => null);
}

function photoMatchesProperty(photo, propertyId, propertyGlobalId) {
  const pid = trimRef(propertyId);
  const gid = trimRef(propertyGlobalId);
  const rowPid = trimRef(photo?.property_id);
  const rowGid = trimRef(photo?.property_global_id);
  if (pid && rowPid && rowPid === pid) return true;
  if (gid && rowGid && rowGid === gid) return true;
  return false;
}
...
async function requirePropertyAccess(req, res, propertyId) {
  ...
  return { ctx, propertyId: resolvedId, propertyGlobalId: getResolvedPropertyGlobalId(property), property, propertyRef: rawPropertyRef };
}
...
async function listPhotoRows(propertyId, propertyGlobalId = '') {
  const ref = trimRef(propertyId);
  const gid = trimRef(propertyGlobalId);
  if (!ref) return [];
  await syncLegacyPropertyPhotos(ref, gid);
  let rows = await supabaseFetch(`/rest/v1/property_photos?select=*&property_id=eq.${encodeURIComponent(ref)}&deleted_at=is.null&order=sort_order.asc,created_at.asc`).catch(() => []);
  rows = Array.isArray(rows) ? rows : [];
  if ((!rows || !rows.length) && gid) {
    rows = await supabaseFetch(`/rest/v1/property_photos?select=*&property_global_id=eq.${encodeURIComponent(gid)}&deleted_at=is.null&order=sort_order.asc,created_at.asc`).catch(() => []);
    rows = Array.isArray(rows) ? rows : [];
  }
  const seen = new Set();
  return rows.filter((row) => {
    const id = trimRef(row?.id);
...
async function getActivePhotoCount(propertyId, propertyGlobalId = '') {
  const rows = await listPhotoRows(propertyId, propertyGlobalId);
...
module.exports = {
  ...
  makeId,
  getActivePhotoCount,
  syncLegacyPropertyPhotos,
  photoMatchesProperty,
  parsePropertyId,
  looksLikeUuid,
  send,
};
