...
const { requireCtxTierWrite } = require('./_lib/admin-tier');
...
async function handleActivityLog(req, res) {
  ...
  if (req.method === 'POST') {
    // admin 인 경우 tier 체크: list/master 만 활동 로그 작성 가능 (BASIC 차단)
    // staff 는 자체 권한 (자기 물건만) — 호출부에서 그대로 작동
    if (!requireCtxTierWrite(ctx, 'properties', res)) return;
    try {
...
async function handleSupabaseWrite(req, res) {
  ...
  // admin tier 체크 (2026-05-08): admin 만 추가 검증, staff 는 통과 (자체 권한 로직 적용)
  if (!requireCtxTierWrite(ctx, 'properties', res)) return;
...
async function handlePhotoAction(req, res, action) {
  ...
  if (!access) return;
  // list 는 보기이므로 모든 admin tier 통과. CUD 는 admin 인 경우 tier 체크 (BASIC 차단)
  if (action !== 'list' && !requireCtxTierWrite(access.ctx, 'properties', res)) return;
  try {
    if (action === 'list') {
