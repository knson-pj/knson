...
// 온비드 공매 입찰결과 API 통합
// ─────────────────────────────────────────────────────────────────
const ONBID_API_ENDPOINT = 'https://apis.data.go.kr/B010003/OnbidCltrBidRsltDtlSrvc2/getCltrBidRsltDtl2';
const ONBID_TPS_INTERVAL_MS = 120;  // 안전 간격 (10 TPS 한도의 80% 수준)
const ONBID_MAX_ITEMS_PER_RUN = 500; // 한 번에 너무 많이 호출하면 서버리스 타임아웃. 배치 단위 제한.
const ONBID_STATUS_MAP = {
  '낙찰': '낙찰',
  '유찰': '유찰',
  '취소': '취하',
};

function getOnbidServiceKey() {
  const key = String(process.env.ONBID_SERVICE_KEY || '').trim();
  return key || '';
}

function parseOnbidDate(str) {
  // yyyyMMddHHmm 또는 yyyyMMdd → YYYY-MM-DD
...
async function callOnbidApi(cltrMngNo) {
  const key = getOnbidServiceKey();
  if (!key) throw new Error('ONBID_SERVICE_KEY 환경변수가 설정되지 않았습니다.');
  const params = new URLSearchParams({
    serviceKey: key,
    pageNo: '1',
    numOfRows: '1',
    resultType: 'json',
    cltrMngNo: String(cltrMngNo || '').trim(),
  });
  const url = `${ONBID_API_ENDPOINT}?${params.toString()}`;
  const res = await fetch(url, { method: 'GET' });
  const text = await res.text();
  if (!res.ok) {
    throw new Error(`온비드 API HTTP ${res.status}: ${text.slice(0, 200)}`);
  }
  let data = null;
  try { data = text ? JSON.parse(text) : null; } catch { throw new Error('온비드 API 응답 파싱 실패'); }
  const header = data?.response?.header || {};
  const code = String(header.resultCode || '').trim();
  if (code === '03') return { nodata: true }; // NODATA_ERROR
  if (code !== '00') {
    return { error: true, code, msg: header.resultMsg || '' };
  }
  const items = data?.response?.body?.items?.item;
  const item = Array.isArray(items) ? items[0] : (items || null);
  if (!item) return { nodata: true };
  return {
    raw: item,
    statusKr: ONBID_STATUS_MAP[String(item.pbctStatNm || '').trim()] || null,
    statusCode: String(item.pbctStatCd || '').trim(),
    scfbAmt: item.scfbAmt ? Number(String(item.scfbAmt).replace(/[^0-9.]/g, '')) || null : null,
    opbdCmptnDt: parseOnbidDate(item.opbdCmptnDt || item.cltrOpbdDt),
    apslEvlAmt: item.apslEvlAmt ? Number(item.apslEvlAmt) || null : null,
    ratio: item.apslPrcCtrsScfbPrcRto ? Number(item.apslPrcCtrsScfbPrcRto) || null : null,
  };
}

async function fetchOnbidPendingProperties() {
  // 공매 중 result_status 가 아직 비어있는 물건만 대상
  // (이미 낙찰/유찰/취소 등록된 건은 스킵)
  const select = 'id,item_no,global_id,result_status';
  const path = `/rest/v1/properties?source_type=eq.onbid&result_status=is.null&select=${encodeURIComponent(select)}&limit=${ONBID_MAX_ITEMS_PER_RUN}`;
  const data = await supabaseRest(path);
  return Array.isArray(data) ? data : [];
}

async function updatePropertyResult(propertyId, resultStatus, resultPrice, resultDate) {
  const patch = {
  ...
  await supabaseRest(`/rest/v1/properties?id=eq.${encodeURIComponent(propertyId)}`, {
    method: 'PATCH',
    json: patch,
...
async function insertOnbidSyncLog(row) {
  try {
    await supabaseRest('/rest/v1/onbid_sync_logs', {
      method: 'POST',
      json: row,
      headers: { Prefer: 'return=minimal' },
    });
  } catch (e) { /* 로그 실패해도 본 작업은 계속 */ }
}

async function syncOnbidResults({ triggeredBy = 'manual', userId = null, itemIds = null } = {}) {
  const startedAt = Date.now();
  if (!getOnbidServiceKey()) {
    return { ok: false, message: 'ONBID_SERVICE_KEY 환경변수가 설정되지 않았습니다.' };
  }
  if (!hasSupabaseAdminEnv()) {
    return { ok: false, message: 'Supabase 환경이 구성되지 않았습니다.' };
  }

  // 대상 수집: itemIds 지정 시 그것만, 아니면 result_status=null 인 공매 전부 (배치 제한)
  let targets = [];
  if (Array.isArray(itemIds) && itemIds.length) {
    const inList = itemIds.map((v) => `"${String(v).replace(/"/g, '')}"`).join(',');
    const path = `/rest/v1/properties?source_type=eq.onbid&id=in.(${encodeURIComponent(inList)})&select=id,item_no,result_status`;
    const data = await supabaseRest(path);
    targets = Array.isArray(data) ? data : [];
  } else {
    targets = await fetchOnbidPendingProperties();
  }
  ...
  for (const prop of targets) {
    const itemNo = String(prop.item_no || '').trim();
    if (!itemNo) { errorCount++; continue; }
    try {
      const result = await callOnbidApi(itemNo);
      if (result.nodata) {
        nodataCount++;
      } else if (result.error) {
        errorCount++;
        if (errorSamples.length < 5) errorSamples.push({ itemNo, code: result.code, msg: result.msg });
      } else {
        successCount++;
        if (result.statusKr) {
          await updatePropertyResult(prop.id, result.statusKr, result.scfbAmt, result.opbdCmptnDt);
          updatedCount++;
        }
      }
    } catch (err) {
      errorCount++;
      if (errorSamples.length < 5) errorSamples.push({ itemNo, error: String(err.message || err).slice(0, 200) });
    }
    // TPS 간격 (10 TPS 한도 준수)
    await new Promise((r) => setTimeout(r, ONBID_TPS_INTERVAL_MS));
  }

  const durationMs = Date.now() - startedAt;

  // 이력 기록 (비동기, 실패해도 결과 반환)
  insertOnbidSyncLog({
    triggered_by: triggeredBy,
    triggered_user: userId,
    target_count: targetCount,
    success_count: successCount,
    nodata_count: nodataCount,
    error_count: errorCount,
    updated_count: updatedCount,
    duration_ms: durationMs,
    error_detail: errorSamples.length ? { samples: errorSamples } : null,
  }).catch(() => {});

  return {
    ok: true,
    targetCount, successCount, nodataCount, errorCount, updatedCount,
    durationMs,
    errors: errorSamples,
  };
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    // mode=onbid_sync: 온비드 공매 입찰결과 API 호출 → DB 업데이트
    const postMode = String(body?.mode || '').trim().toLowerCase();
    if (postMode === 'onbid_sync') {
      try {
        const triggeredBy = String(body?.triggeredBy || 'manual').trim() || 'manual';
        const itemIds = Array.isArray(body?.itemIds) ? body.itemIds : null;
        const result = await syncOnbidResults({
          triggeredBy,
        ...
        return send(res, err?.status || 500, {
          ok: false,
          message: err?.message || '온비드 동기화 중 오류가 발생했습니다.',
        });
      }
    }
